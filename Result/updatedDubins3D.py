...
def dubins_curve_length(curve_3d):
    if curve_3d is None or len(curve_3d) < 2:
        return 0
    totalLength = 0
    for i in range(1, len(curve_3d)):
        totalLength += dist_3d(curve_3d[i-1][0], curve_3d[i-1][1], curve_3d[i-1][2],
                               curve_3d[i][0], curve_3d[i][1], curve_3d[i][2])
    return totalLength

def RRT_Dubins_Realtime(img, start, end, stepSize, dubins_radius, plotter=None):
    """
    ...
    while not pathFound and iteration < max_iterations:
        ...
        if nodeCon:
            ...
            if directCon:
                ...
                totalDistance += dubins_curve_length(final_3d)
                ...
                totalDistance += dubins_curve_length(solution_array)
    ...
    return pathFound, node_list, totalDistance, totalTime
